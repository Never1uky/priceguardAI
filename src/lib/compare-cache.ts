...
export function hasStoredCompareOffers(product: CompareProduct): boolean {
  return offersFromCompareProduct(product).some((o) => isOfferWithPrice(o));
}
...
export function shouldRunCompare(product: CompareProduct, force = false): boolean {
  if (force) return true;
  if (!hasStoredCompareOffers(product)) return true;
