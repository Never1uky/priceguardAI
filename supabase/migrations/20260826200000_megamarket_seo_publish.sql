-- MEGA-8: SEO publish allowlist DB CHECKs for megamarket.
...
comment on constraint seo_product_pages_marketplace_check on public.seo_product_pages is
  'CORE trio + megamarket (MEGA-8 SEO publishAllowed). Gates still apply per page.';
...
-- offers_snapshot cross-MP mapping may include Mega as source or target
...
alter table public.cross_market_mapping
  add constraint cross_market_mapping_source_marketplace_check check (
    source_marketplace in ('wildberries', 'ozon', 'yandex_market', 'megamarket')
  );
...
alter table public.cross_market_mapping
  add constraint cross_market_mapping_target_marketplace_check check (
    target_marketplace in ('wildberries', 'ozon', 'yandex_market', 'megamarket')
  );
