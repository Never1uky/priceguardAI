/**
 * SEO slug + analysis hash (docs/SEO_PRODUCT_PAGES.md).
...
export function seoProductKey(marketplace: string, productId: string): string {
...
/** Heuristic brand/model from product title when features unavailable. */
export function guessBrandModelFromTitle(title: string): { brand?: string; model?: string } {
  const parts = title.trim().split(/\s+/).filter(Boolean);
  if (parts.length === 0) return {};
  if (parts.length === 1) return { brand: parts[0] };
  return {
    brand: parts[0],
    model: parts.slice(1, Math.min(parts.length, 5)).join(' '),
  };
}
