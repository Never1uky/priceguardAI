...
| `agent_searches` | user_id, query text, constraints/result jsonb | Per-user shopping-agent search history | Lives with the account | `on delete cascade` from `auth.users` | Not in `purge_privacy_ttl_data()` — user queries, not cache (`search_results_cache` is the 6h SERP TTL) |
